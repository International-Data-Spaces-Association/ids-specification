...
- A [`Dataspace Registry` or `Dataspace Registration Service`](./terminology.md#dataspace-registration-service) manages the registered `Participant Agents` in a `Dataspace`. Registration and unregistration processes may be different depending on various aspects.
