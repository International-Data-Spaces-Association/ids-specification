...
A push transfer is when the provider connector initiates the sending of data to a consumer endpoint. For example, after the consumer has issued an `TransferRequestMessage,` the
...
A pull transfer is when the consumer connector initiates retrieval of data from a provider endpoint. For example, after the provider has issued an `TransferProcessStart,`
